...
public class MAN_DIC_AMBITOSDao extends GenericDaoJpaImpl<MAN_DIC_AMBITOS, Integer> implements GenericDao<MAN_DIC_AMBITOS, Integer> {
	...
	public List<MAN_DIC_AMBITOS> getall2() {
		...
		Query query = entityManager.createNativeQuery("select a.ID,NOME, STRING_AGG(c.EMAIL,',') as EMAIL,a.OBRIGA_DOCUMENTOS,a.AMBITO_IT,a.NOTIFICA_PREVENTIVAS from MAN_DIC_AMBITOS a "
				+ "LEFT JOIN MAN_DIC_AMBITO_UTILIZADORES b on a.ID = b.ID_EQUIPA LEFT JOIN GER_UTILIZADORES c on b.ID_UTILIZADOR = c.ID_UTILIZADOR GROUP BY a.ID,NOME,a.OBRIGA_DOCUMENTOS,a.AMBITO_IT,a.NOTIFICA_PREVENTIVAS");
