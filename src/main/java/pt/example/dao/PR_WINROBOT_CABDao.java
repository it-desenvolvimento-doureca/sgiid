...
public class PR_WINROBOT_CABDao extends GenericDaoJpaImpl<PR_WINROBOT_CAB, Integer>
	...
	public List<PR_WINROBOT_CAB> getbyestado(String estado) {
		...
		Query query = entityManager.createNativeQuery(";WITH CTE_REFERENCIAS AS (  SELECT ID_CAB, "
				...
				+ " FROM PR_WINROBOT_ARTICLES WITH (NOLOCK) GROUP BY ID_CAB ), CTE_RACKS AS ( "
				+ " SELECT ID_CAB,  STRING_AGG(RACK_CODE, ',') AS RACKS   FROM PR_WINROBOT_RACKS WITH (NOLOCK) "
				+ " GROUP BY ID_CAB ) SELECT a.*,  b.REFERENCIA AS referencias, "
				+ " c.RACKS AS racks FROM PR_WINROBOT_CAB a WITH (NOLOCK) "
