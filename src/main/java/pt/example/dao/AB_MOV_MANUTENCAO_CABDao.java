...
public class AB_MOV_MANUTENCAO_CABDao extends GenericDaoJpaImpl<AB_MOV_MANUTENCAO_CAB, Integer>
	...
	public List<AB_MOV_MANUTENCAO_CAB> getbyidbanho(Integer idbanho, Integer inicio, Integer fim, Integer id_man) {
		Query query = entityManager.createNativeQuery(
				"SELECT a.ID_MANUTENCAO_CAB,DATA_PLANEAMENTO,HORA_PLANEAMENTO, ( SELECT COUNT(*) AS totalPayments FROM AB_MOV_MANUTENCAO_CAB where ID_BANHO = :idbanho and ID_MANUTENCAO_CAB not in (:id_man) ) "
						+ "FROM ( SELECT *, ROW_NUMBER() OVER (ORDER BY ID_MANUTENCAO_CAB desc) as row FROM AB_MOV_MANUTENCAO_CAB where ID_BANHO = :idbanho and ID_MANUTENCAO_CAB not in (:id_man) ) a "
						+ "inner join AB_MOV_MANUTENCAO b on a.ID_MANUTENCAO = b.ID_MANUTENCAO "
						+ "WHERE row > :inicio and row <= :fim order by b.DATA_PLANEAMENTO desc");
