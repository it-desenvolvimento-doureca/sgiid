...
public class RH_FUNCIONARIOSDao extends GenericDaoJpaImpl<RH_FUNCIONARIOS, Integer>
	...
	public List<RH_FUNCIONARIOS> getGeral(String datageral, String Ativo, String Operario, String SECTOR_ACESSO,
		...
		/*
		 * Query query = entityManager.createNativeQuery("DECLARE @DATA date = '" +
		 * datageral + "'; " +
		 ...
		 * ") and ((not " + Operario + " is not null) or (a.COD_FUNCIONARIO  = " +
		 * Operario + " )) " + querywhere +
		...
		Query query = entityManager.createNativeQuery("DECLARE @DATA date = '" + datageral + "'; "
				...
				+ "where (a.COD_SECTOR in (" + SECTOR_ACESSO + ") " + queryallsector + ") and ((not " + Operario
	...
	public List<RH_FUNCIONARIOS> getOperacoes(String data1, String data2, String Ativo, String Operario,
		...
		Query query = entityManager.createNativeQuery("DECLARE @DATA date = '" + data1 + "'; "
				+ "DECLARE @DATA2 date = '" + data2 + "';  "
				+ " select distinct COD_FUNCIONARIO,NOME,ATIVO,LOCAL,RESPONSAVEL,COD_SECTOR,DES_SECTOR,chefe,datdeb,proref,prodes1,opecod,opedes,quant,TempoPrep,TempoExec,"
				+ " CASE WHEN (TempoPausas = 0 or TempoPausas is null) and  (TempoTotalPausa/totalproref) >0 THEN TempoTotal - TempoPausas + (TempoTotalPausa/totalproref) ELSE TempoTotal END TempoTotal ,heudeb,heufin,ofnum,cadencia, "
				+ " CASE WHEN (TempoPausas = 0 or TempoPausas is null) and  (TempoTotalPausa/totalproref) >0 THEN (TempoTotalPausa/totalproref) ELSE TempoPausas END TempoPausas "
				+ " from ( select   COD_FUNCIONARIO,NOME,ATIVO,LOCAL,RESPONSAVEL,COD_SECTOR,DES_SECTOR,chefe,datdeb,proref,prodes1,opecod,opedes,quant,TempoPrep,TempoExec,"
				+ "	CASE WHEN TempoPausas /*/COUNT(proref) OVER(PARTITION BY COD_FUNCIONARIO,datdeb,heudeb)*/ + TempoPrep + TempoExec !=  TempoTotal THEN TempoTotal + TempoPausas /*/COUNT(proref) OVER(PARTITION BY COD_FUNCIONARIO,datdeb,heudeb)*/ ELSE TempoTotal END TempoTotal,heudeb,heufin,ofnum,cadencia,"
				+ " TempoPausas /*/COUNT(proref) OVER(PARTITION BY COD_FUNCIONARIO,datdeb,heudeb)*/  as TempoPausas,TempoTotalPausa,COUNT(proref) OVER(PARTITION BY COD_FUNCIONARIO,datdeb,heudeb) totalproref "
				...
				+ " ,PecasHora,typof,totalproref " + "from ( "
				...
				+ "where xva1.ofdnumenr = tabx.ofdnumenr  and tabx.svanumenr = xva2.svanumenr) else total_pecas END * case when  total_horas < 0  and total_pecas >=0  then -1 else 1 end total_pecas,total_horas "
				...
				+ " ,AVG(CASE  WHEN typof IN ('OFC1', 'OFC2','OFCF') THEN  total_pecas_linha ELSE case when /*total_pecas <> total_trabalho and total_pecas = 0 and*/ ID_OF_CAB is not null THEN total_trabalho else total_pecas END  END)total_pecas "
				+ "	,AVG(CASE  WHEN typof IN ('OFC1', 'OFC2','OFCF') THEN  total_horas_linha ELSE total_horas END)total_horas	,AVG(qtt3) qtt3 "
				...
				+ "	) tag left join SGIID.dbo.RP_OF_OP_FUNC b on ID_UTZ_CRIA = rescod AND  ((DATA_INI = datdeb AND heudeb = HORA_INI AND heufin = HORA_FIM)  OR (DATA_INI_M1 = datdeb AND heudeb = HORA_INI_M1 AND heufin = HORA_FIM_M1)  OR (DATA_INI_M2 = datdeb AND heudeb = HORA_INI_M2 AND heufin = HORA_FIM_M2) )  "
				+ " left join SGIID.dbo.RP_OF_OP_CAB c on b.ID_OP_CAB = c.ID_OP_CAB "
				+ ") taby where (typof in ('OFC1', 'OFC2','OFCF') or (total_pecas is not null) and total_horas > 0 or total_pecas = qtt3) group by ofdnumenr,svanumenr,svanumori) tabx "
				+ ")  txa on txa.SVANUMENR = (CASE WHEN t.SVANUMORI = 0 THEN CASE WHEN typof IN ('OFC1', 'OFC2','OFCF') THEN t.SVANUMENR ELSE t.SVANUMENR2 END ELSE t.SVANUMORI END) "
				...
				+ query_tipo_cadencia + " "
				+ "where /*a.proref is not null and total is not null and*/ (b.COD_SECTOR in (" + SECTOR_ACESSO + ") "
				+ queryallsector + ") and ((not " + sectornull + " is not null) or (c.COD_SECTOR  in (" + Sector
				...
				+ ") c " + "left join ( "
				+ " SELECT  t.rescod, t.datdeb as datedeb2,t.heudeb as heudeb2,SUM(t.stpse+t.stpsp) TempoTotalPausa,SUM(CASE WHEN x.ARRCOD in ('TECH') THEN 1 ELSE 0 END ) TOTAL_P "
	...
	public List<RH_FUNCIONARIOS> getSectores_(String data1, String Ativo, String Operario, String Sector,
			String SECTOR_ACESSO, String tipo_cadencia) {
		
		List<RH_FUNCIONARIOS> data = entityManager
				.createNativeQuery("EXEC RH_GET_SECTOR "
						+ ":DATA,   :Ativo, :Sector, :SECTOR_ACESSO, :Operario, :tipo_cadencia")
				.setParameter("DATA", data1).setParameter("Ativo", Ativo)
				.setParameter("tipo_cadencia", tipo_cadencia).setParameter("Sector", Sector)
				.setParameter("Operario", Operario)
				.setParameter("SECTOR_ACESSO", SECTOR_ACESSO).getResultList();
	...
	public List<RH_FUNCIONARIOS> getSectoresComparativa_(String data1, String Ativo, String Sector, String SECTOR_ACESSO,
			Boolean ADMIN, String tipo_cadencia) {

		List<RH_FUNCIONARIOS> data = entityManager
				.createNativeQuery("EXEC RH_GET_COMPARATIVA_SECTORES "
						+ ":DATA, :Ativo, :Sector, :SECTOR_ACESSO, :ADMIN,:tipo_cadencia")
				.setParameter("DATA", data1).setParameter("Ativo", Ativo).setParameter("Sector", Sector).setParameter("ADMIN", ADMIN ? 1 : 0)
				.setParameter("SECTOR_ACESSO", SECTOR_ACESSO).setParameter("tipo_cadencia", tipo_cadencia).getResultList();
