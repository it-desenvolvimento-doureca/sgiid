...
public class PR_PLANEAMENTO_PRODUCAO_ANALISESDao extends GenericDaoJpaImpl<PR_PLANEAMENTO_PRODUCAO_ANALISES, Integer>
	...
	public List<PR_PLANEAMENTO_PRODUCAO_ANALISES> getall2() {
		...
		Query query = entityManager.createNativeQuery("Select a.ID_PLANEAMENTO_PRODUCAO_ANALISES,a.DATA_CRIA,a.SEMANA,a.ANO,a.N_SEMANAS, "
				...
				+ " (select CONCAT(CONCAT(CONCAT(CONCAT(CONCAT(x.ID_PLANEAMENTO_PRODUCAO_CAB,' | '), CAST (x.DATA_CRIA as DATE )) +' | ' ,x.DATA_MRP) ,' | '), x.N_MRP)  from PR_PLANEAMENTO_PRODUCAO_CAB x where x.ID_PLANEAMENTO_PRODUCAO_CAB = a.ID_PLANO_LINHA_2) as PLANO_LINHA_2,a.TIPO_ANALISE "
				+ ",b.NOME_UTILIZADOR  from PR_PLANEAMENTO_PRODUCAO_ANALISES a  LEFT JOIN GER_UTILIZADORES b on a.UTZ_CRIA = b.ID_UTILIZADOR where a.ATIVO = 1 order by a.ID_PLANEAMENTO_PRODUCAO_ANALISES DESC");
		List<PR_PLANEAMENTO_PRODUCAO_ANALISES> data = query.getResultList();
		return data;
