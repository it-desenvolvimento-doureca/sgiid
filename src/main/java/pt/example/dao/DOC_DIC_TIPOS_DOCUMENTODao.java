...
public class DOC_DIC_TIPOS_DOCUMENTODao extends GenericDaoJpaImpl<DOC_DIC_TIPOS_DOCUMENTO, Integer>
	...
	public List<DOC_DIC_TIPOS_DOCUMENTO> getallUser(Integer id) {
		...
		Query query = entityManager.createNativeQuery("Select a.* from DOC_DIC_TIPOS_DOCUMENTO a "
				+ "where a.INATIVO != 1 and a.ID in (select b.ID_TIPO_DOCUMENTO from DOC_DIC_TIPOS_DOCUMENTO_UTZ b where b.ID_UTZ = :id or (select top 1 ADMIN from GER_UTILIZADORES c where c.ID_UTILIZADOR = :id) = 1) ",
				DOC_DIC_TIPOS_DOCUMENTO.class);
		query.setParameter("id", id);
		List<DOC_DIC_TIPOS_DOCUMENTO> data = query.getResultList();
		return data;
