	...
	public List<PE_MOV_CAB> getallbyTIPO(String tipo, String emAtraso,String user) {
		...
		Query query = entityManager.createNativeQuery(
				" DECLARE @emAtraso bit = " + emAtraso + " " + "select "
						...
						+ ",ISNULL(g.PERCENTAGEM_CONCLUSAO,0) conclusaoacao  "
						
						+ ",AVG( "
						+ "    CASE  "
						+ "        WHEN YEAR(b.DATA_CRIA) <= xa.ANO_PLANO  "
						+ "             AND g.DATA_CONCLUSAO IS NOT NULL  "
						+ "             AND YEAR(g.DATA_CONCLUSAO) < xa.ANO_PLANO  "
						+ "        THEN NULL "
						+ "        ELSE ISNULL(g.PERCENTAGEM_CONCLUSAO, 0) "
						+ "    END "
						+ ") OVER(PARTITION BY a.ID_PLANO_CAB,xa.ID) AS conclusaoplano "
						+ " "
						+ " ,CASE  "
						+ "    WHEN (select COUNT(DISTINCT t1.ID_PLANO_CAB) from PA_MOV_CAB t1  "
						+ "          where t1.ID_PLANO_CAB in (select vp.ID_PLANO_CAB from PE_PLANOS_ASSOCIADOS vp WHERE vp.ID_PLANO_ESTRATEGICO = xa.ID)) = 0  "
						+ "    THEN 0  "
						+ " "
						+ "    WHEN (select COUNT(DISTINCT t1.ID_PLANO_CAB) from PA_MOV_CAB t1  "
						+ "          where t1.ID_PLANO_CAB in (select vp.ID_PLANO_CAB from PE_PLANOS_ASSOCIADOS vp WHERE vp.ID_PLANO_ESTRATEGICO = xa.ID)) = 1  "
						+ "    THEN   "
						+ "        AVG( "
						+ "            CASE  "
						+ "                WHEN YEAR(b.DATA_CRIA) <= xa.ANO_PLANO  "
						...
						+ " "
						+ "    ELSE   "
						+ "        ISNULL((select AVG( PERCENTAGEM_CONCLUSAO) from ( "
						+ "            select AVG( PERCENTAGEM_CONCLUSAO) PERCENTAGEM_CONCLUSAO   "
						+ "            from ( "
						+ "                select t1.ID_PLANO_CAB, "
						+ "                    AVG( "
						+ "                        CASE  "
						+ "                            WHEN YEAR(t2.DATA_CRIA) <= xa.ANO_PLANO  "
						+ "                                 AND t3.DATA_CONCLUSAO IS NOT NULL  "
						+ "                                 AND YEAR(t3.DATA_CONCLUSAO) < xa.ANO_PLANO  "
						+ "                            THEN NULL "
						+ "                            ELSE ISNULL(t3.PERCENTAGEM_CONCLUSAO, 0) "
						+ "                        END "
						...
						+ "                where t1.ID_PLANO_CAB in (select vp.ID_PLANO_CAB from PE_PLANOS_ASSOCIADOS vp WHERE vp.ID_PLANO_ESTRATEGICO = xa.ID)   "
						+ "            ) t4  "
						...
						+ "left join PE_PLANOS_ASSOCIADOS xb WITH (NOLOCK) on xa.ID = xb.ID_PLANO_ESTRATEGICO "
						+ "left join PA_MOV_CAB a WITH (NOLOCK) on xb.ID_PLANO_CAB = a.ID_PLANO_CAB   "
						+ "left join GER_DEPARTAMENTO xe WITH (NOLOCK) on xa.DEPARTAMENTO = xe.ID" + " left join PA_MOV_LINHA b WITH (NOLOCK) on a.ID_PLANO_CAB = b.ID_PLANO_CAB "
