	...
	public List<PA_MOV_CAB> getbyidPlanoEstrategico(String tipo, Integer id,Integer user) {
		...
		Query query = entityManager.createNativeQuery(
				"select a.ID_PLANO_CAB,a.DATA_CRIA,a.DATA_OBJETIVO,a.AMBITO,a.ORIGEM,(select NOME_UTILIZADOR from GER_UTILIZADORES x where x.ID_UTILIZADOR = a.UTZ_CRIA) as UTZ_CRIA,a.DESCRICAO,a.ESTADO "
						...
						+ ",CASE WHEN EXISTS(SELECT 1 FROM PA_MOV_CAB_HISTORICO h WHERE h.ID_PLANO_CAB = a.ID_PLANO_CAB AND NOT (h.TIPO_ALTERACAO = 'ESTADO' AND h.VALOR_ANTERIOR IS NULL)) OR EXISTS(SELECT 1 FROM PA_MOV_LINHA_HISTORICO lh INNER JOIN PA_MOV_LINHA pl ON lh.ID_PLANO_LINHA = pl.ID_PLANO_LINHA WHERE pl.ID_PLANO_CAB = a.ID_PLANO_CAB) THEN 1 ELSE 0 END as TEM_HISTORICO "
	...
	public List<PA_MOV_CAB> getallbyTIPO(String tipo, String fastresponse, String emAtraso,String user) {
		...
		Query query = entityManager.createNativeQuery(
				"DECLARE @fastresponse bit = " + fastresponse + " DECLARE @emAtraso bit = " + emAtraso + " " + "select "
						...
						+ ",CASE WHEN EXISTS(SELECT 1 FROM PA_MOV_CAB_HISTORICO h WHERE h.ID_PLANO_CAB = a.ID_PLANO_CAB AND NOT (h.TIPO_ALTERACAO = 'ESTADO' AND h.VALOR_ANTERIOR IS NULL)) OR EXISTS(SELECT 1 FROM PA_MOV_LINHA_HISTORICO lh INNER JOIN PA_MOV_LINHA pl ON lh.ID_PLANO_LINHA = pl.ID_PLANO_LINHA WHERE pl.ID_PLANO_CAB = a.ID_PLANO_CAB) THEN 1 ELSE 0 END as TEM_HISTORICO_PLANO "
						+ ",CASE WHEN EXISTS(SELECT 1 FROM PA_MOV_LINHA_HISTORICO lh WHERE lh.ID_PLANO_LINHA = b.ID_PLANO_LINHA) THEN 1 ELSE 0 END as TEM_HISTORICO_LINHA "
	...
	public List<PA_MOV_CAB> getallbyTIPOaccoes(String tipo, String fastresponse, String emAtraso,String DATA_INI,String DATA_FIM) {
		...
		Query query = entityManager.createNativeQuery(
				"DECLARE @fastresponse bit = " + fastresponse + " DECLARE @emAtraso bit = " + emAtraso + " " + "select "
						...
						+ ",CASE WHEN a.UNIDADE = 1 THEN 'Formariz' WHEN   a.UNIDADE = 2 THEN 'São Bento' ELSE '' END as UNIDADES "
