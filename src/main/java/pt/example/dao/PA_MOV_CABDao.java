...
public class PA_MOV_CABDao extends GenericDaoJpaImpl<PA_MOV_CAB, Integer> implements GenericDao<PA_MOV_CAB, Integer> {
	...
	public List<PA_MOV_CAB> getall() {
		...
		Query query = entityManager.createQuery("Select a,(select COR from AB_DIC_LINHA WHERE ID_LINHA  = a.ID_LINHA) as LINHA"
	...
	public List<PA_MOV_CAB> getallbyTIPO(String tipo,String fastresponse, String emAtraso) {
		...
		if(fastresponse.equals("true")){ fastresponse  ="1"; } else{ fastresponse  ="0";}
		if(emAtraso.equals("true")){ emAtraso  ="1"; } else{ emAtraso  ="0";}
		
		Query query = entityManager.createNativeQuery("DECLARE @fastresponse bit = "+fastresponse+" DECLARE @emAtraso bit = "+emAtraso+" "
				+ "select "
				...
				+ ",b.ESTADO as ESTADO_ACAO,b.fastresponse ,(select sd.DESCRICAO from PA_DIC_AMBITOS sd where sd.ID_AMBITO = a.AMBITO) as AMBITO_DESC, f.DESCRICAO as TIPO_ACAO_DESC,g.ID_TAREFA "
				+ "from PA_MOV_CAB a " 
	...
	public List<PA_MOV_CAB> getallbyTIPOaccoes(String tipo,String fastresponse, String emAtraso) {
		...
		Query query = entityManager.createNativeQuery("DECLARE @fastresponse bit = "+fastresponse+" DECLARE @emAtraso bit = "+emAtraso+" "
				...
				+ "GER_DEPARTAMENTO i on u.ID_DEPARTAMENTO = i.ID where t.ID_UTILIZADOR = b.RESPONSAVEL) "
				+ "from PA_MOV_CAB a " 
