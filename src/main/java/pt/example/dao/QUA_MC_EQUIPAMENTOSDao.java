...
public class QUA_MC_EQUIPAMENTOSDao extends GenericDaoJpaImpl<QUA_MC_EQUIPAMENTOS, Integer>
	...
	// C12 = data em que passou a obsoleto; C13 = data aquisição; C14 = em calibração externa em atraso.
	public List<Object[]> getlista() {
		String sql =
			...
			"      ELSE DATEADD(MONTH, itv.MESES, lcp.DATA_CALIBRACAO) END AS C11, a.DATA_OBSOLETO AS C12, a.DATA_AQUISICAO AS C13, " +
			" CASE WHEN a.OBSOLETO = 0 AND a.CALIBR_EXTERNA = 1 AND (lcp_ext.DATA_CALIBRACAO IS NULL OR DATEADD(MONTH, a.INTERCALIB_EXT_MESES, lcp_ext.DATA_CALIBRACAO) < GETDATE()) THEN 1 ELSE 0 END AS C14, a.N_SERIE AS C15 " +
			...
			"  FROM QUA_MC_MOV_CALIB_EQUIP_DET d INNER JOIN QUA_MC_MOV_CALIB_EQUIP h ON d.ID_CALIB_EQUIP = h.ID_CALIB_EQUIP " +
			"  LEFT JOIN QUA_MC_DIC_TIPO_CALIBRACAO tc ON tc.ID_TIPO_CALIBRACAO = d.ID_TIPO_CALIBRACAO " +
			"  WHERE h.ID_EQUIPAMENTO = a.ID_EQUIPAMENTO AND UPPER(tc.TIPO_CALIB_EQUP) LIKE '%EXTERN%' " +
			"  ORDER BY d.DATA_CALIBRACAO DESC, d.ID_CALIB_EQUIP_DET DESC ) lcp_ext " +
