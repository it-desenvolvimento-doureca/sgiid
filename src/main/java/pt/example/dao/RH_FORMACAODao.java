...
public class RH_FORMACAODao extends GenericDaoJpaImpl<RH_FORMACAO, Integer>
	...
	public List<RH_FORMACAO> getall2(String id) {
		...
		Query query = entityManager.createNativeQuery("declare @id varchar(25) =  " + id
				...
				+ "from RH_FORMACAO a " + "left join RH_DIC_AREA_FORMACAO b on a.ID_CRITERIOS_AVALIACAO = b.ID "
				+ "left join RH_DIC_CRITERIOS_AVALIACAO c on a.ID_CRITERIOS_AVALIACAO = c.ID "
				+ "left join RH_DIC_ENTIDADE_FORMADORA d on a.ID_AREA_FORMACAO = d.ID "
	...
	public List<Object[]> RH_GET_FORMACOES_TOTAIS(String ano, String utilizador, String DATA_INICIO, String DATA_FIM) {
		...
		Query query = entityManager.createNativeQuery("declare @DATA_INICIO varchar(25) =  " + DATA_INICIO
				...
				+ "sum(case when CAST(GETDATE() as date) > DATA_FIM THEN DURACAO else 0 end) total_Horas_Terminadas, "
				+ "sum(case when CAST(GETDATE() as date) >= DATA_INICIO and  CAST(GETDATE() as date) <= DATA_FIM THEN DURACAO else 0 end) total_Horas_Em_Progresso, "
				+ "sum(case when DATA_INICIO > CAST(GETDATE() as date) THEN DURACAO else 0 end) total_Horas_Por_Iniciar "
				+ " from RH_FORMACAO a  where a.ATIVO = 1 and YEAR(DATA_INICIO) = @ano "
