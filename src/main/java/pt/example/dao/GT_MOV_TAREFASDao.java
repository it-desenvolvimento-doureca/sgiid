	...
	public List<GT_MOV_TAREFAS> getbyidFiltros(List<HashMap<String, String>> data2) {
		...
		Query query = entityManager.createNativeQuery(
				"DECLARE @TABELA_TEMP TABLE(ID_RECLAMACAO int,CLIENTE varchar(max),REF varchar(max),NOME_REF varchar(max),ID varchar(max),RESPONSAVEL varchar(max),OBRIGA_EVIDENCIAS bit,TIPO varchar(max),MODULO int,DEPARTAMENTO int,SUB_MODULO varchar(2)) "
						+ "INSERT @TABELA_TEMP "
						+ "select e.ID_RECLAMACAO,e.NOME_CLIENTE as CLIENTE,e.REFERENCIA as REF,e.DESIGNACAO_REF as NOME_REF,d.ID,e.UTZ_RESPONSAVEL as RESPONSAVEL,OBRIGA_EVIDENCIAS,d.TIPO,5,null,'C' SUB_MODULO from RC_MOV_RECLAMACAO_PLANOS_ACCOES d inner join RC_MOV_RECLAMACAO e on d.ID_RECLAMACAO = e.ID_RECLAMACAO "
						+ " union select e.ID_RECLAMACAO,e.NOME_FORNECEDOR as CLIENTE,e.REFERENCIA as REF,e.DESIGNACAO_REF as NOME_REF,d.ID,e.UTZ_RESPONSAVEL as RESPONSAVEL,OBRIGA_EVIDENCIAS,d.TIPO,5,null,'F' SUB_MODULO from RC_MOV_RECLAMACAO_FORNECEDOR_PLANOS_ACCOES d inner join RC_MOV_RECLAMACAO_FORNECEDOR e on d.ID_RECLAMACAO = e.ID_RECLAMACAO "
						
						+ "union select c.ID_AMOSTRA,null,c.REFERENCIA,c.DESCRICAO,b.ID_AMOSTRA_ACCAO,a.UTZ_CRIA,null,null,10,null,'A' SUB_MODULO from GT_MOV_TAREFAS a left join PR_AMOSTRAS_ACCOES b on a.ID_CAMPO = b.ID_AMOSTRA_ACCAO "
						...
						+ "union select c.ID_PLANO_CAB,null,b.REFERENCIA,b.DESIGN_REFERENCIA,b.ID_PLANO_LINHA,a.UTZ_CRIA,null,null,13,c.DEPARTAMENTO_ORIGEM,'PA' SUB_MODULO from GT_MOV_TAREFAS a  "
