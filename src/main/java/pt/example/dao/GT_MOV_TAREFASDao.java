...
public class GT_MOV_TAREFASDao extends GenericDaoJpaImpl<GT_MOV_TAREFAS, Integer>
	...
	public List<GT_MOV_TAREFAS> getbyidUser(Integer id, List<HashMap<String, String>> data2) {
		...
		Query query = entityManager.createNativeQuery("Select  a.ID_ACCAO  "
				...
				+ ",(select count(*) from GT_MOV_TAREFAS where ID_MODULO = 8 and SUB_MODULO = 'SUG'   and ( ( UTZ_TIPO = 'u' and (UTZ_ID = 1 OR UTZ_ENCAMINHADO  =1 and 1 = CASE WHEN UTZ_ENCAMINHADO is not null and UTZ_ENCAMINHADO  != 1 THEN 0 ELSE 1 END )))  and ESTADO ='P' and ID_ACCAO = a.ID_ACCAO ) as TAREFAS_NAO_LIDAS_SUGESTOES "
				+ ",(select count(*) from GT_MOV_TAREFAS where ID_MODULO = 8 and SUB_MODULO = 'SUG'  and ( ( UTZ_TIPO = 'u' and (UTZ_ID = 1 OR UTZ_ENCAMINHADO  =1 and 1 = CASE WHEN UTZ_ENCAMINHADO is not null and UTZ_ENCAMINHADO  != 1 THEN 0 ELSE 1 END )))  and ESTADO in ('P','L','E') and ID_ACCAO = a.ID_ACCAO ) as TOTAL_TAREFAS_SUGESTOES "
				+ ",(select count(*) from GT_MOV_TAREFAS where ID_MODULO = 8 and SUB_MODULO = 'SUG'  and ( ( UTZ_TIPO = 'u' and (UTZ_ID = 1 OR UTZ_ENCAMINHADO  =1 and 1 = CASE WHEN UTZ_ENCAMINHADO is not null and UTZ_ENCAMINHADO  != 1 THEN 0 ELSE 1 END )))  and ESTADO in ('P','L','E') and ID_ACCAO = a.ID_ACCAO AND UTZ_ENCAMINHADO is not null) as TAREFAS_ENCAMINHADAS_SUGESTOES "
				+ ",(select count(*) from GT_MOV_TAREFAS where DATA_FIM<GETDATE() AND ID_MODULO = 8 and SUB_MODULO = 'SUG'  and ( ( UTZ_TIPO = 'u' and (UTZ_ID = 1 OR UTZ_ENCAMINHADO  =1 and 1 = CASE WHEN UTZ_ENCAMINHADO is not null and UTZ_ENCAMINHADO  != 1 THEN 0 ELSE 1 END )))  and ESTADO in ('P','L','E') and ID_ACCAO = a.ID_ACCAO ) as TOTAL_TAREFAS_ATRASO_SUGESTOES "
