...
public class GT_MOV_TAREFASDao extends GenericDaoJpaImpl<GT_MOV_TAREFAS, Integer>
	...
	public List<GT_MOV_TAREFAS> getbyidUser(Integer id, List<HashMap<String, String>> data2) {
		...
		Query query = entityManager.createNativeQuery("Select  a.ID_ACCAO  "
				...
				+ ",(select count(*) from GT_MOV_TAREFAS where ID_MODULO= 13 and SUB_MODULO = 'PA'  " + query_utilizador+ " and ESTADO ='P' and ID_ACCAO = a.ID_ACCAO ) as TAREFAS_NAO_LIDAS_PLANOS "
				+ ",(select count(*) from GT_MOV_TAREFAS where ID_MODULO= 13 and SUB_MODULO = 'PA' " + query_utilizador+ " and ESTADO in ('P','L','E') and ID_ACCAO = a.ID_ACCAO ) as TOTAL_TAREFAS_PLANOS "
				+ ",(select count(*) from GT_MOV_TAREFAS where ID_MODULO= 13 and SUB_MODULO = 'PA' " + query_utilizador+ " and ESTADO in ('P','L','E') and ID_ACCAO = a.ID_ACCAO AND UTZ_ENCAMINHADO is not null) as TAREFAS_ENCAMINHADAS_PLANOS "
	...
	public List<GT_MOV_TAREFAS> getbyidFiltros(List<HashMap<String, String>> data2) {
		...
		Query query = entityManager.createNativeQuery(
				"DECLARE @TABELA_TEMP TABLE(ID_RECLAMACAO int,CLIENTE varchar(max),REF varchar(max),NOME_REF varchar(max),ID varchar(max),RESPONSAVEL varchar(max),OBRIGA_EVIDENCIAS bit,TIPO varchar(max),MODULO int,DEPARTAMENTO int) "
						...
						+ "left join PR_AMOSTRAS_CAB c on b.ID_AMOSTRA = c.ID_AMOSTRA where ((not " + varquery6+ " != 0) or (ID_TAREFA = " + id + ")) and a.ID_MODULO = 10 "
						+ "union select c.ID_PLANO_CAB,null,c.REFERENCIA,c.DESIGN_REFERENCIA,b.ID_PLANO_LINHA,a.UTZ_CRIA,null,null,13,c.DEPARTAMENTO_ORIGEM from GT_MOV_TAREFAS a  "
						+ "left join PA_MOV_LINHA b on a.ID_CAMPO = b.ID_PLANO_LINHA left join PA_MOV_CAB c on b.ID_PLANO_CAB = c.ID_PLANO_CAB  where ((not " + varquery6+ " != 0) or (ID_TAREFA = " + id + ")) and a.ID_MODULO = 13 "
						...
						+ "(select EMAIL from GER_UTILIZADORES where ID_UTILIZADOR = b.RESPONSAVEL) as UTILIZADOR_ORIGEM_EMAIL,b.TIPO,a.ID_MODULO,a.SUB_MODULO,(select DESCRICAO from GER_DEPARTAMENTO where ID = b.DEPARTAMENTO) NOME_DEPARTAMENTO, a.MOTIVO_REJEICAO,a.JUSTIFICACAO_ALTERACAO_ESTADO  "
